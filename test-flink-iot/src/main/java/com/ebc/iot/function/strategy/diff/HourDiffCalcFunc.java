...
public class HourDiffCalcFunc implements CalcFuncStrategy {
    ...
    public void open(RuntimeContext ctx){
        ...
        // 设置 TTL 为 2 小时 (或者稍微长一点，比如 24 小时以防万一)
        // 这里为了安全起见，设为 1 天，因为小时数据量也不大
    ...
    public void calculate(DevicePointRule rule, CalculationContext ctx) throws Exception {
        ...
        String hourStr = dt.format(DateTimeFormatter.ofPattern("yyyyMMddHH"));
        
        String startKey = rule.getSourcePointCode() + "_hour_" + hourStr + "_start";
